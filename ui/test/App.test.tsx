...
const aboutRoutes = () => [
  {
    method: 'GET',
    match: /\/version$/,
    json: { bundleName: 'AAD Synchronizer', bundleVendor: 'SBB', bundleVersion: '1.0.0' },
  },
...
afterEach(() => {
  ...
  document.cookie = 'aad-synchronizer-dev-scope=; path=/; max-age=0';
});
...
describe('App router', () => {
  ...
  it('renders the About page for ?feature=about', async () => {
    ...
    expect(document.body.textContent).toContain('AAD Synchronizer');
...
describe('accessibility', () => {
  ...
  it('has no WCAG A/AA violations on the Landing stub with an error alert', async () => {
    installFetchMock([
      { method: 'GET', match: /\/polarion\/rest\/v1\/projects/, respond: () => jsonResponse({}, 401) },
    ]);
    window.history.replaceState({}, '', '?');
